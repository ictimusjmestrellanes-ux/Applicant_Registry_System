...
@section('content')
    ...
    <div class="applicant-index-page container-fluid py-4 px-md-4 px-xl-5">
                    ...
                    <div class="col-lg-8">
                        ...
                        <div class="search-shell">
                            ...
                            <input type="text" name="search" class="form-control index-field-control"
                                placeholder="Type applicant name, phone number, or gender..."
                                value="{{ $searchTerm }}">
                        </div>
                    </div>
                    <div class="col-lg-4">
        ...
        <section class="records-card">
            <div class="section-head mb-3">
                <div>
                    ...
                    <p class="section-copy mb-0">Review profile details and document readiness before opening the full applicant workspace.</p>
                </div>
                @if($searchTerm !== '')
                        ...
                        @forelse($applicants as $applicant)
                            ...
                            <tr>
                                ...
                                <td class="location-cell">
                                    ...
                                    <div class="applicant-meta mt-1">{{ $applicant->barangay ?: 'Barangay not set' }}, {{ $applicant->city ?: 'City not set' }}</div>
                                </td>
                                <td>
                                    <div class="requirement-card">
                                        <div class="requirement-meta"><span>Mayor's Permit</span><span>{{ $permitUploaded }}/{{ $permitTotal }}</span></div>
                                        <div class="progress requirement-progress">
                                            <div class="progress-bar {{ $permitPercent == 100 ? 'bg-success' : ($permitPercent > 0 ? 'bg-warning' : 'bg-danger') }}" style="width: {{ $permitPercent }}%;"></div>
                                        </div>
                                ...
                                <td>
                                    <div class="requirement-card">
                                        <div class="requirement-meta"><span>Mayor's Clearance</span><span>{{ $clearanceUploaded }}/{{ $clearanceTotal }}</span></div>
                                        <div class="progress requirement-progress">
                                            <div class="progress-bar {{ $clearancePercent == 100 ? 'bg-success' : ($clearancePercent > 0 ? 'bg-warning' : 'bg-danger') }}" style="width: {{ $clearancePercent }}%;"></div>
                                        </div>
                                ...
                                <td>
                                    <div class="requirement-card">
                                        <div class="requirement-meta"><span>Mayor's Referral</span><span>{{ $referralUploaded }}/2</span></div>
                                        <div class="progress requirement-progress">
                                            <div class="progress-bar {{ $referralPercent == 100 ? 'bg-success' : ($referralPercent > 0 ? 'bg-warning' : 'bg-danger') }}" style="width: {{ $referralPercent }}%;"></div>
                                        </div>
                                ...
                                <td class="text-center">
                                    <div class="action-stack">
                                        <a href="{{ route('applicants.edit', $applicant->id) }}" class="btn btn-sm btn-view" title="View Applicant"><i class="bi bi-eye-fill"></i></a>
                                        <form action="{{ route('applicants.destroy', $applicant->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to archive this applicant?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-archive" title="Archive Applicant"><i class="bi bi-archive-fill"></i></button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
                ...
                @forelse($applicants as $applicant)
                    ...
                    <article class="mobile-record-card">
                        <div class="mobile-record-head">
                            <div>
                                <div class="applicant-name">{{ $fullName }}</div>
                                <div class="applicant-meta">#{{ $applicant->id }} • {{ $applicant->created_at->format('M d, Y') }}</div>
                            </div>
                        ...
                        <div class="mobile-meta-line"><i class="bi bi-telephone-fill me-2"></i>{{ $applicant->contact_no ?: 'No contact number' }}</div>
                        <div class="mobile-meta-line"><i class="bi bi-geo-alt-fill me-2"></i>{{ $applicant->barangay ?: 'Barangay not set' }}, {{ $applicant->city ?: 'City not set' }}</div>
        ...
        <div class="pagination-wrap mt-4">
            <div class="pagination-summary">Showing {{ $showingFrom }} to {{ $showingTo }} of {{ number_format($totalApplicants) }} applicants</div>
    ...
    <style>
        ...
        .index-kicker, .overview-label, .field-label, .hero-side-label {
            display: inline-flex;
            align-items: center;
        ...
        .index-copy, .section-copy, .overview-copy, .applicant-meta, .pagination-summary, .requirement-note, .hero-side-copy { color: #64748b; }
        .hero-side-card, .overview-card, .filter-card, .records-card {
        ...
        .hero-side-label { color: #93c5fd; margin-bottom: 0.7rem; }
        .hero-side-title {
            font-size: 1.8rem;
            font-weight: 800;
            line-height: 1.1;
        ...
        .hero-side-actions, .filter-actions {
            display: flex;
            gap: 0.75rem;
            flex-wrap: wrap;
        }
        ...
        .view-pill strong, .applicant-name, .section-title { color: #0f172a; font-weight: 800; }
        .view-pill small { display: block; color: #64748b; margin-top: 0.2rem; }
        .view-pill-icon {
            width: 50px;
            height: 50px;
            display: inline-flex;
        ...
        .view-pill-icon.archive { background: rgba(148, 163, 184, 0.18); color: #334155; }
        .overview-card, .filter-card, .records-card { padding: 1.5rem; }
        .overview-card { height: 100%; position: relative; overflow: hidden; }
        .overview-card::after {
            content: "";
            position: absolute;
            right: -18px;
        ...
        .index-field-control { min-height: 54px; border: none; background: transparent; }
        .index-field-control:focus { box-shadow: none; background: transparent; }
        .results-chip, .search-chip, .mini-pill {
            display: inline-flex;
            align-items: center;
            border-radius: 999px;
            font-size: 0.82rem;
        ...
        .results-chip { padding: 0.55rem 0.9rem; background: #eff6ff; color: #1d4ed8; }
        .search-chip { padding: 0.55rem 0.9rem; background: #f1f5f9; color: #334155; }
        .success-banner { border-radius: 18px; padding: 1rem 1.25rem; }
        ...
        .applicants-table tbody td { padding: 1rem; border-top: 1px solid #eef2f7; vertical-align: middle; }
        .applicants-table tbody tr:hover { background: #f8fbff; }
        .table-id, .created-date { color: #475569; font-weight: 500; }
        .mini-pill-row { display: flex; gap: 0.45rem; flex-wrap: wrap; }
        ...
        .requirement-progress { height: 8px; background: #e2e8f0; border-radius: 999px; overflow: hidden; }
        .requirement-note { margin-top: 0.45rem; font-size: 0.76rem; }
        .action-stack, .mobile-actions { display: inline-flex; align-items: center; gap: 0.5rem; }
        ...
        .btn-view { background: rgba(37, 99, 235, 0.12); color: #1d4ed8; }
        .btn-view:hover { background: #dbeafe; color: #1e40af; }
        .btn-archive { background: rgba(239, 68, 68, 0.12); color: #dc2626; }
        .btn-archive:hover { background: #fee2e2; color: #991b1b; }
        .mobile-records { display: grid; gap: 1rem; }
        .mobile-record-card { padding: 1rem; border-radius: 20px; background: #f8fafc; border: 1px solid #e2e8f0; }
        .mobile-record-head {
            display: flex;
            justify-content: space-between;
            gap: 1rem;
        ...
        .mobile-progress-list { display: grid; gap: 0.85rem; margin-top: 1rem; }
        .empty-state {
            display: flex;
        ...
        .empty-state i { font-size: 2.2rem; color: #94a3b8; }
        .pagination-wrap {
            display: flex;
            justify-content: space-between;
            align-items: center;
        ...
        .page-link { border-radius: 10px; border: 1px solid #dbeafe; color: #334155; }
        .page-item.active .page-link { background: #2563eb; border-color: #2563eb; }
        @media (max-width: 1199.98px) { .hero-side-actions, .filter-actions { justify-content: flex-start; } }
        @media (max-width: 991.98px) {
            .index-hero, .overview-card, .filter-card, .records-card { padding: 1.25rem; }
            .view-switcher { grid-template-columns: 1fr; }
            .section-head, .mobile-record-head { flex-direction: column; align-items: flex-start; }
        }
        @media (max-width: 767.98px) {
            .filter-actions, .hero-side-actions { flex-direction: column; }
            .index-btn-primary, .index-btn-secondary { width: 100%; }
